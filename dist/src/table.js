...
class Table {
    ...
    /** 指定したデータでテーブルを再作成する */
    resetTable(records) {
        const data = records.map((r) => Table.isTRow(r) ? r.hash : r)
            .map((hash, i) => {
            const data = {
        ...
        if (data.length > 0) {
            // シートをクリア
            Sheets.Spreadsheets?.Values?.batchClear({ ranges: [`${this.sheet}!${this.head_col}${this.head_row + 1}:ZZ`] }, this.ss.getId());
            // シートを上書き
